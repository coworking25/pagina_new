-- =====================================================
-- SISTEMA DE GESTIÓN DE CLIENTES - SCHEMA COMPLETO
-- =====================================================
...
-- =====================================================
-- 4. TABLA CLIENT_COMMUNICATIONS (Comunicaciones)
-- =====================================================
...
-- =====================================================
-- 5. TABLA CLIENT_DOCUMENTS (Documentos)
-- =====================================================
...
CREATE TRIGGER update_communications_updated_at BEFORE UPDATE ON client_communications FOR EACH ROW EXECUTE FUNCTION update_updated_at_column();
...
CREATE TRIGGER update_alerts_updated_at BEFORE UPDATE ON client_alerts FOR EACH ROW EXECUTE FUNCTION update_updated_at_column();

-- =====================================================
-- TRIGGERS AUTOMÁTICOS PARA ALERTAS
-- =====================================================

-- Función para crear alertas de pagos vencidos
...
-- Trigger para contratos por vencer
...
-- =====================================================
-- FUNCIÓN PARA GENERAR PAGOS AUTOMÁTICOS
-- =====================================================
...
CREATE POLICY "Usuarios pueden ver clientes asignados" ON clients
    FOR SELECT USING (
        assigned_advisor_id = auth.uid() OR 
        EXISTS (SELECT 1 FROM user_profiles WHERE user_id = auth.uid() AND role IN ('admin', 'manager'))
    );

CREATE POLICY "Usuarios pueden insertar clientes" ON clients
    FOR INSERT WITH CHECK (
        assigned_advisor_id = auth.uid() OR 
        EXISTS (SELECT 1 FROM user_profiles WHERE user_id = auth.uid() AND role IN ('admin', 'manager'))
    );

CREATE POLICY "Usuarios pueden actualizar sus clientes" ON clients
    FOR UPDATE USING (
        assigned_advisor_id = auth.uid() OR 
        EXISTS (SELECT 1 FROM user_profiles WHERE user_id = auth.uid() AND role IN ('admin', 'manager'))
    );
...
-- (Las políticas específicas se pueden ajustar según los roles y permisos)
...
LEFT JOIN properties prop ON ct.property_id = prop.id
...
-- ✅ 6 tablas interconectadas para gestión completa de clientes
-- ✅ Índices optimizados para consultas rápidas
-- ✅ Triggers automáticos para alertas y timestamps
-- ✅ Función para generar pagos automáticos
